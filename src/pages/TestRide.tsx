...
const TestRide: React.FC = () => {
  ...
  const steps = ['Select Rider & Driver', 'Configure Ride Details', 'View Fare Breakdown'];

  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [fetchedRiders, fetchedDrivers, fetchedPricingRules] = await Promise.all([
          getDrivers(),
          getDrivers(),
  ...
  useEffect(() => {
    ...
    const rule = pricingRules.find(r => r.rideTypeId === selectedRideType) || pricingRules[0];
    
    const baseFare = rule.basePrice;
    
    // Calculate billable distance (only charge for distance beyond the minimum billable distance)
    const billableDistance = Math.max(0, distance - rule.minimumBillableDistance);
  ...
  const completeRide = async () => {
    ...
    const createdRide: Ride = {
      ...
      createdAt: new Date(Date.now() - (duration + 5) * 60 * 1000).toISOString()
    };
                ...
                <Card elevation={3}>
                  <CardContent>
                    ...
                    <List disablePadding>
                            ...
                            <>
                              ...
                              {pricingRules.length > 0 && (
                                <Typography variant="caption" display="block" color="text.secondary">
                                  First {pricingRules.find(r => r.rideTypeId === selectedRideType)?.minimumBillableDistance || 0}km included in base price
                                </Typography>
                              )}
                            </>
                          } 
                      ...
                      <ListItem>
                        <ListItemText 
                          primary={<Typography variant="subtitle1" fontWeight="bold">Total Fare</Typography>} 
                          secondary={<Typography variant="subtitle1">{formatCurrency(fareBreakdown.totalFare)}</Typography>} 
                        />
                      </ListItem>
                    </List>
                  </CardContent>
                    ...
                    <List disablePadding>
                      <ListItem>
                        ...
                        <ListItemText 
                          ...
                          secondary={<Typography color="info.main">{formatCurrency(fareBreakdown.commissionAmount)}</Typography>} 
                        />
                      </ListItem>
                      ...
                      <ListItem>
                        ...
                        <ListItemText 
                          ...
                          secondary={<Typography color="success.main" fontWeight="medium">{formatCurrency(fareBreakdown.driverEarnings)}</Typography>} 
                        />
                      </ListItem>
          ...
          <Box>
            <Paper elevation={3} sx={{ p: 3, mb: 3, bgcolor: 'success.light' }}>
              ...
              <Typography variant="body1" paragraph>
                The test ride has been successfully completed. The driver earned {formatCurrency(fareBreakdown.driverEarnings)} after the {fareBreakdown.commissionPercentage}% platform commission of {formatCurrency(fareBreakdown.commissionAmount)}.
              </Typography>
                    ...
                    <Grid container spacing={2}>
                      <Grid item xs={6}>
                        <Paper sx={{ p: 2, bgcolor: 'info.light', color: 'info.contrastText', textAlign: 'center' }}>
                          ...
                          <Typography variant="h6">{formatCurrency(fareBreakdown.commissionAmount)}</Typography>
                        </Paper>
                      </Grid>
                      
                      <Grid item xs={6}>
                        <Paper sx={{ p: 2, bgcolor: 'success.light', color: 'success.contrastText', textAlign: 'center' }}>
                          <Typography variant="body2">Driver Earnings</Typography>
                          <Typography variant="h6">{formatCurrency(fareBreakdown.driverEarnings)}</Typography>
                        </Paper>
                      </Grid>
                    </Grid>
                  </CardContent>
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Typography variant="body1" color="text.secondary" paragraph>
        Simulate a ride to see how the 15% platform commission is calculated and how much the driver earns.
      </Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
          ...
          {activeStep === 3 ? (
            ...
            <Button 
              variant="contained" 
              onClick={handleNext}
              disabled={loading || (!selectedRider && activeStep === 0) || (!selectedDriver && activeStep === 0)}
            >
