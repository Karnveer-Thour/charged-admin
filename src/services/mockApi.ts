...
const generateRiders = (count: number): Rider[] => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const rewardPoints = Math.floor(Math.random() * totalRides * 10) + totalRides * 5;
    const signedUpDate = new Date(Date.now() - Math.random() * 90 * 24 * 60 * 60 * 1000).toISOString();
    const lastRideDate = Math.random() > 0.1 
...
const generateDrivers = (count: number): Driver[] => {
  ...
  const vehicleMakes = ['Toyota', 'Honda', 'Tesla', 'Ford', 'Chevrolet', 'Nissan', 'BMW'];
  const vehicleModels = ['Corolla', 'Civic', 'Model 3', 'Focus', 'Cruze', 'Sentra', 'X3'];
  const vehicleColors = ['Black', 'White', 'Silver', 'Red', 'Blue', 'Grey'];
  const documentStatuses: DocumentStatus[] = ['pending', 'approved', 'rejected', 'expired', 'notSubmitted'];
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const generateDocument = (type: DocumentType): DriverDocument => {
      ...
      const dateSubmitted = status !== 'notSubmitted' 
        ? new Date(Date.now() - Math.random() * 90 * 24 * 60 * 60 * 1000).toISOString()
        : undefined;
      const dateReviewed = (status === 'approved' || status === 'rejected') 
        ? new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString()
        : undefined;
      const expiryDate = (status === 'approved' || status === 'expired')
        ? new Date(Date.now() + (status === 'expired' ? -1 : 1) * Math.random() * 365 * 24 * 60 * 60 * 1000).toISOString()
        : undefined;
      ...
      return {
        ...
        notes: status === 'rejected' ? 'Document unclear or incomplete' : undefined,
        fileUrl: status !== 'notSubmitted' ? `https://example.com/documents/${type}-${i}.pdf` : undefined
      };
    };
...
const generateRides = (count: number): Ride[] => {
  const rides: Ride[] = [];
  const statuses: Ride['status'][] = ['pending', 'in-progress', 'completed', 'cancelled'];
  ...
  const generateLocation = (): Location => ({
    ...
    longitude: -122.4194 + (Math.random() - 0.5) * 0.1,
    address: [
      ['Market St', 'Mission St', 'Valencia St', 'Castro St', 'Divisadero St'][Math.floor(Math.random() * 5)],
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const riderId = mockRiders[Math.floor(Math.random() * mockRiders.length)].id;
    const driverId = mockDrivers[Math.floor(Math.random() * mockDrivers.length)].user_id;
    ...
    const rule = pricingRules.find(r => r.rideTypeId === rideType) || pricingRules[0];
    let fare = rule.basePrice + (distance * rule.pricePerKm) + (duration * rule.pricePerMinute);
    
    // Round to 2 decimal places
    fare = Math.round(fare * 100) / 100;
    
    const startTime = status === 'pending' ? undefined : new Date(new Date(createdAt).getTime() + 5 * 60 * 1000).toISOString();
    
    const endTime = status === 'completed' 
      ? new Date(new Date(startTime || createdAt).getTime() + duration * 60 * 1000).toISOString()
      : undefined;
    
    const cancelTime = status === 'cancelled'
      ? new Date(new Date(startTime || createdAt).getTime() + Math.random() * 10 * 60 * 1000).toISOString()
      : undefined;
    
    const cancelReason = status === 'cancelled'
      ? ['Change of plans', 'Driver taking too long', 'Emergency', 'Booked by mistake'][Math.floor(Math.random() * 4)]
      : undefined;
    
    const driverDistanceAtCancel = status === 'cancelled'
      ? Math.floor(Math.random() * 200)
      : undefined;
    
    const refunded = status === 'cancelled' && driverDistanceAtCancel !== undefined && driverDistanceAtCancel > 70
      ? Math.random() > 0.5
      : false;
...
const generateDashboardStats = (): DashboardStats => {
  ...
  const avgCommissionRate = pricingRules.reduce((sum, rule) => sum + rule.commissionPercentage, 0) / pricingRules.length;
  const totalCommission = totalRevenue * (avgCommissionRate / 100);
  
  const totalRefunds = cancelledRides.filter(ride => ride.refunded).reduce((sum, ride) => sum + ride.fare, 0);
  const totalPoints = mockRiders.reduce((sum, rider) => sum + rider.rewardPoints, 0);
  
  const ridesByType = [
    { type: 'Electric', count: mockRides.filter(ride => ride.rideType === 'electric').length },
    { type: 'Regular', count: mockRides.filter(ride => ride.rideType === 'regular').length },
    { type: 'SUV', count: mockRides.filter(ride => ride.rideType === 'suv').length }
  ];
...
export const mockApi = {
  ...
  updateDriverDocument: async (
    ...
    updatedDocuments[documentIndex] = {
      ...updatedDocuments[documentIndex],
      ...updates,
      dateReviewed: updates.status ? new Date().toISOString() : updatedDocuments[documentIndex].dateReviewed,
      reviewedBy: updates.status ? 'Admin User' : updatedDocuments[documentIndex].reviewedBy
    };
  ...
  // Refunds
  updateRefundStatus: async (rideId: string, refunded: boolean, reason?: string): Promise<Ride> => {
    await new Promise(resolve => setTimeout(resolve, 800));
  ...
  uploadDocumentForDriver: async (driverId: string, documentType: DocumentType, fileData: any, notes?: string) => {
    try {
  ...
  notifyDriverAboutDocument: async (driverId: string, documentType: DocumentType) => {
    try {
      // Simulate server delay
      await new Promise(resolve => setTimeout(resolve, 500));
      
      // In a real app, this would send a push notification to the driver
      console.log(`Notification sent to driver ${driverId} about ${documentType} upload`);
